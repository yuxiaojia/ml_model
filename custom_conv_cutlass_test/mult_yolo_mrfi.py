#!/usr/bin/env python3
"""
YOLOv9-C COCO Validation with MRFI Fault Injection
Standalone script - evaluates YOLOv9-C with MRFI fault injection.
"""
...
def evaluate_yolo_with_mrfi(
    ...
    """Evaluate YOLOv9 on COCO validation set with MRFI fault injection.
    ...
    # Apply MRFI fault injection if target_layer specified
...
def main():
    parser = argparse.ArgumentParser(description='YOLOv9-C COCO Validation with MRFI')
    parser.add_argument('--layer', type=str, default="model.0.conv", help='Target layer for fault injection')
    parser.add_argument('--count', type=int, default=1000, help='Number of faults to inject')
    parser.add_argument('--factor', type=float, default=1000.0, help='Fault perturbation factor')
    args = parser.parse_args()

    print("YOLOv9-C COCO Validation with MRFI")
    print("=" * 60)
    ...
    # Evaluate
    map50, map, num_images, num_instances = evaluate_yolo_with_mrfi(
        weights=weights,
        data_yaml=data_yaml,
